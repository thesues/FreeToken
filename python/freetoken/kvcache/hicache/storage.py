"""HiCache L3 storage interface — ported verbatim from sglang.

Upstream: sglang/python/sglang/srt/mem_cache/hicache_storage.py @ a757c1e3f
Kept byte-compatible on purpose: the whole point of this port is that storage
backends written against sglang's `HiCacheStorage` (autumn, mooncake, hf3fs, …)
load into FreeToken unmodified. Do not "clean up" these signatures — a drifted
ABC silently downgrades every out-of-tree backend to the slow v0 path or breaks
it outright.

Two sglang couplings were cut, nothing else:
...
class HiCacheFile(HiCacheStorage):
    ...
    def __init__(
        ...
        self.file_path = os.environ.get("FREETOKEN_HICACHE_FILE_DIR") or file_path
